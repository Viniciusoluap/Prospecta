...
export default function AdminFinanceiro() {
  ...
  const { data: transactions = [], refetch } =
    trpc.financialTransactions.list.useQuery({
  ...
  const { data: distributions = [], refetch: refetchDist } =
    trpc.partnerDistributions.list.useQuery({ status: "pending" });

  const markPaidMutation = trpc.partnerDistributions.markPaid.useMutation({
    onSuccess: () => {
      toast.success("Distribuição marcada como paga!");
      refetchDist();
    },
    onError: e => toast.error(e.message),
  });
    ...
    <div className="min-h-screen bg-[#1A2332] text-white">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">
                Receitas, despesas e distribuições
              </p>
            </div>
          </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1"
                      placeholder="ex: obra, taxa, comissão"
                    />
                  </div>
                  <div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card
            className={`bg-[#2C3E50] ${profit >= 0 ? "border-[#C9A961]/30" : "border-red-500/30"}`}
          >
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                <DollarSign
                  className={`h-8 w-8 ${profit >= 0 ? "text-[#C9A961]" : "text-red-400"}`}
                />
                <div>
                  <p className="text-gray-400 text-sm">Lucro Líquido</p>
                  <p
                    className={`text-2xl font-bold ${profit >= 0 ? "text-[#C9A961]" : "text-red-400"}`}
                  >
          ...
          <Card className="bg-[#2C3E50] border-[#C9A961]/20">
            <CardHeader>
              <CardTitle className="text-[#C9A961] flex items-center gap-2">
                <Users className="h-5 w-5" /> Distribuições Pendentes para
                Sócios
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {distributions.map((dist: any) => (
                  <div
                    key={dist.id}
                    className="flex items-center justify-between p-3 bg-[#1A2332] rounded-lg"
                  >
                    <div>
                      <p className="font-medium text-white">
                        {dist.notes || `Distribuição #${dist.id}`}
                      </p>
                      <p className="text-sm text-gray-400">
                        {dist.percentage}% • {formatCurrencyBR(dist.amount)}
                      </p>
                    </div>
                    <Button
                      size="sm"
                        ...
                        <div className="flex items-center gap-2">
                          <Badge
                            ...
                            style={{
                              ...
                              color: typeInfo?.color?.replace("text-", ""),
                            }}
                          >
                            {typeInfo?.label}
                          </Badge>
