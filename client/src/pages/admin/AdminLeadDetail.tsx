...
export default function AdminLeadDetail() {
  ...
  const { data: lead, refetch } = trpc.leads.getById.useQuery(
    { id: leadId },
    {
      enabled: !!leadId,
    }
  ...
  useEffect(() => {
    if (lead && Object.keys(editForm).length === 0) {
      setEditForm({
        ...
        state: lead.state || "",
        income: lead.income || "",
        fgts: (lead as any).fgts || false,
        pisFgts: lead.pisFgts || "",
        hasSpouse: lead.hasSpouse || false,
        spouseName: lead.spouseName || "",
        incomeComposition: lead.incomeComposition || false,
        ...
        simulationValue: lead.simulationValue || "",
        approvedValue: lead.approvedValue || "",
        interest: (lead as any).interest || "financing",
        type: lead.type || "new_lead",
      });
    }
  }, [lead]);
  ...
  const addDocMutation = trpc.leads.addDocument.useMutation({
    onSuccess: () => {
      ...
      refetch();
      setAddDocOpen(false);
      setDocForm({ type: "rg", fileName: "", notes: "" });
    },
    onError: e => toast.error(e.message),
  });
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                <Badge
                  className={`${STAGE_COLORS[lead.stage] || "bg-gray-500"} text-white text-xs`}
                >
                ...
                <div className="space-y-4 mt-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        value={editForm.simulationValue || ""}
                        onChange={e =>
                          setEditForm(f => ({
                            ...f,
                            simulationValue: e.target.value,
                          }))
                        }
                        className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1"
                      />
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        value={editForm.approvedValue || ""}
                        onChange={e =>
                          setEditForm(f => ({
                            ...f,
                            approvedValue: e.target.value,
                          }))
                        }
                        className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1"
                      />
                    ...
                    onClick={() =>
                      updateMutation.mutate({
                        ...
                        income: editForm.income
                          ? parseFloat(editForm.income)
                          : undefined,
                        simulationValue: editForm.simulationValue
                          ? parseFloat(editForm.simulationValue)
                          : undefined,
                        approvedValue: editForm.approvedValue
                          ? parseFloat(editForm.approvedValue)
                          : undefined,
                      })
                    }
                    disabled={updateMutation.isPending}
          ...
          <div className="flex items-center gap-1 min-w-max">
            {STAGES.map((stage, idx) => (
              <div key={stage.key} className="flex items-center gap-1">
                ...
                {idx < STAGES.length - 1 && (
                  <ChevronRight
                    className={`h-3 w-3 ${idx < currentStageIdx ? "text-[#C9A961]" : "text-gray-700"}`}
                  />
                )}
              </div>
            ))}
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <Card className="bg-[#2C3E50] border-[#C9A961]/20">
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <InfoRow
                label="Responsável"
                value={
                  lead.responsible === "sarah"
                    ? "Sarah"
                    : lead.responsible === "vinicius"
                      ? "Vinicius"
                      : "Bianca"
                }
              />
          ...
          <Card className="bg-[#2C3E50] border-[#C9A961]/20">
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <InfoRow
                label="FGTS"
                value={(lead as any).fgts ? "Sim" : "Não"}
              />
              <InfoRow label="PIS/FGTS" value={lead.pisFgts} />
              <InfoRow
                label="Cônjuge"
                value={
                  lead.hasSpouse ? `Sim — ${lead.spouseName || ""}` : "Não"
                }
              />
              <InfoRow
                label="Composição de Renda"
                value={lead.incomeComposition ? "Sim" : "Não"}
              />
              <InfoRow
                label="CPF Status"
                value={
                  lead.cpfStatus === "clean"
                    ? "Limpo"
                    : lead.cpfStatus === "restricted"
                      ? "Restrito"
                      : "Desconhecido"
                }
              />
              <InfoRow
                label="Interesse"
                value={
                  (lead as any).interest === "house"
                    ? "Casa"
                    : (lead as any).interest === "lot"
                      ? "Lote"
                      : (lead as any).interest === "financing"
                        ? "Financiamento"
                        : "Obra"
                }
              />
              ...
              <InfoRow
                ...
                value={formatCurrencyBR(lead.simulationValue)}
                highlight
              />
              <InfoRow
                label="Valor Aprovado"
                value={formatCurrencyBR(lead.approvedValue)}
                highlight
              />
            </CardContent>
          </Card>
            ...
            <Card className="bg-[#2C3E50] border-[#C9A961]/20">
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="p-3 bg-[#1A2332] rounded-lg space-y-2">
                  <InfoRow
                    label="Renda Total"
                    value={formatCurrencyBR(totalIncome)}
                  />
                  <InfoRow
                    label="Parcela Máx (30%)"
                    value={formatCurrencyBR(maxMonthlyPayment)}
                  />
                  <InfoRow
                    label="Financiamento Est."
                    value={formatCurrencyBR(estimatedFinancing)}
                  />
                  <Separator className="bg-[#C9A961]/20" />
                ...
                <p className="text-gray-400 text-xs">
                  * Estimativa. Banco financia até 80% do menor valor (avaliação
                  ou compra)
                </p>
              </CardContent>
            </Card>
        ...
        <div className="lg:col-span-2 space-y-4">
              ...
              <Dialog open={addDocOpen} onOpenChange={setAddDocOpen}>
                <DialogTrigger asChild>
                  <Button
                    size="sm"
                    variant="outline"
                    className="border-[#C9A961]/30 text-[#C9A961] hover:bg-[#1A2332] h-7"
                  >
              ...
              <div className="space-y-2">
                {DOC_TYPES.map(docType => {
                  ...
                  return (
                    <div
                      key={docType.key}
                      className="flex items-center justify-between p-2 rounded-lg bg-[#1A2332]"
                    >
                      <div className="flex items-center gap-2">
                        <span
                          className={
                            DOC_STATUS_COLORS[doc?.status || "pending"]
                          }
                        >
              ...
              {lead.activities && lead.activities.length > 0 ? (
                <div className="space-y-3">
                  {lead.activities.map((activity: any) => (
                    <div
                      key={activity.id}
                      className="flex gap-3 p-3 rounded-lg bg-[#1A2332]"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-xs text-[#C9A961] font-medium uppercase">
                            {activity.type}
                          </span>
          ...
          {lead.notes && (
            <Card className="bg-[#2C3E50] border-[#C9A961]/20">
              <CardHeader className="pb-3">
                <CardTitle className="text-[#C9A961] text-sm">
                  Observações do Lead
                </CardTitle>
              </CardHeader>
              <CardContent>
...
function InfoRow({
  ...
  return (
    <div className="flex justify-between items-start gap-2">
      ...
      <span
        className={`font-medium text-right ${highlight ? "text-[#C9A961]" : "text-gray-200"} flex items-center gap-1`}
      >
        {icon} {value || "—"}
      </span>
    </div>
