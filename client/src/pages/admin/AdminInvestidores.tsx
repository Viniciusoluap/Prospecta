...
export default function AdminInvestidores() {
  ...
  const addTxMutation = trpc.investors.addTransaction.useMutation({
    onSuccess: () => { toast.success("Transação registrada!"); refetch(); setTxOpen(false); setTxForm({ type: "deposit", amount: "", description: "", dueAt: "", notes: "" }); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const totalInvested = investors.reduce((acc: number, inv: any) => acc + parseFloat(inv.balance || "0"), 0);
      ...
      <div className="bg-[#0F1923] border-b border-[#C9A961]/20 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[#C9A961]">Investidores</h1>
              <p className="text-gray-400 text-sm">Controle de sócios investidores e retornos</p>
            </div>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="bg-[#1A2332] border-[#C9A961]/20 text-white">
              <DialogHeader>
                <DialogTitle className="text-[#C9A961]">Cadastrar Investidor</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 mt-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1" />
                  </div>
                  <div>
                    <Label className="text-gray-300">Taxa de Retorno (%)</Label>
                    <Input type="number" step="0.01" value={form.rate} onChange={e => setForm(f => ({ ...f, rate: e.target.value }))} className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1" placeholder="ex: 1.8" />
                  </div>
                  <div>
                    <Label className="text-gray-300">Saldo Inicial (R$)</Label>
                    <Input type="number" value={form.initialBalance} onChange={e => setForm(f => ({ ...f, initialBalance: e.target.value }))} className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1" />
                  </div>
                </div>
                <div>
                ...
                <Button
                  onClick={() => createMutation.mutate({ ...form, rate: form.rate ? parseFloat(form.rate) : undefined, initialBalance: form.initialBalance ? parseFloat(form.initialBalance) : undefined })}
          ...
          <div className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input type="number" value={txForm.amount} onChange={e => setTxForm(f => ({ ...f, amount: e.target.value }))} className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1" />
              </div>
              <div className="col-span-2">
                <Label className="text-gray-300">Descrição</Label>
                <Input value={txForm.description} onChange={e => setTxForm(f => ({ ...f, description: e.target.value }))} className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1" />
              </div>
              <div>
                <Label className="text-gray-300">Vencimento</Label>
                <Input type="date" value={txForm.dueAt} onChange={e => setTxForm(f => ({ ...f, dueAt: e.target.value }))} className="bg-[#2C3E50] border-[#C9A961]/30 text-white mt-1" />
              </div>
            </div>
            <Button
              onClick={() => selectedInvestor && addTxMutation.mutate({ investorId: selectedInvestor, ...txForm, amount: parseFloat(txForm.amount), dueAt: txForm.dueAt ? new Date(txForm.dueAt) : undefined })}
              disabled={addTxMutation.isPending || !txForm.amount || !txForm.description}
              className="w-full bg-[#C9A961] hover:bg-[#B8985A] text-[#1A2332] font-bold"
            >
              Registrar
            </Button>
        ...
        <Card className="bg-[#2C3E50] border-[#C9A961]/20">
          <CardContent className="pt-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-[#C9A961]">{formatCurrencyBR(totalInvested)}</p>
            </div>
          </CardContent>
        </Card>
              ...
              <Card key={inv.id} className="bg-[#2C3E50] border-[#C9A961]/20">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className={inv.status === "active" ? "bg-green-600 text-white" : "bg-gray-600 text-white"}>
                ...
                <CardContent className="space-y-3">
                  <div className="p-3 bg-[#1A2332] rounded-lg space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-bold text-[#C9A961] text-lg">{formatCurrencyBR(inv.balance)}</span>
                    </div>
                    {inv.rate && (
                      <div className="flex justify-between text-sm">
                  ...
                  {inv.notes && <p className="text-xs text-gray-500 italic">{inv.notes}</p>}
                  <Button
                    size="sm"
                    onClick={() => { setSelectedInvestor(inv.id); setTxOpen(true); }}
