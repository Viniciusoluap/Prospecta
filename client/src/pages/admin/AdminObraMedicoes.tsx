...
import {
  ArrowLeft, Plus, Building2, DollarSign, TrendingUp, CheckCircle2,
  Clock, AlertCircle, Loader2, HardHat, BarChart3
} from "lucide-react";
...
const STATUS_MAP: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  pending:         { label: "Pendente",          color: "bg-gray-600 text-gray-200",   icon: <Clock className="h-3 w-3" /> },
  cef_paid:        { label: "CEF Pagou",         color: "bg-blue-600 text-blue-100",   icon: <DollarSign className="h-3 w-3" /> },
  received:        { label: "Recebido",          color: "bg-green-700 text-green-100", icon: <CheckCircle2 className="h-3 w-3" /> },
  contractor_paid: { label: "Empreiteiro Pago",  color: "bg-yellow-600 text-yellow-100", icon: <HardHat className="h-3 w-3" /> },
};
...
export default function AdminObraMedicoes() {
  ...
  const { data: project, isLoading: loadingProject } = trpc.construction.getProjectDetails.useQuery(
    { projectId },
  ...
  const createMutation = trpc.obraMedicoes.create.useMutation({
    onSuccess: () => {
      ...
      setOpenNew(false);
      setNewMed({ tipo: "pls", dataPrevista: "", valorCef: "", empreiteiroCpf: "", notes: "" });
      refetch();
    },
  ...
  const confirmReceiptMutation = trpc.obraMedicoes.confirmReceipt.useMutation({
    onSuccess: () => { toast.success("Recebimento confirmado!"); refetch(); },
    onError: (e) => toast.error(e.message),
  });

  const payContractorMutation = trpc.obraMedicoes.payContractor.useMutation({
    onSuccess: () => { toast.success("Pagamento registrado!"); setOpenPay(null); setPayAmount(""); refetch(); },
    onError: (e) => toast.error(e.message),
  });
    ...
    <div className="min-h-screen bg-[#1A2332] text-white">
          ...
          <div className="flex gap-3">
            ...
            <Dialog open={openNew} onOpenChange={setOpenNew}>
              ...
              <DialogContent className="bg-[#1A2332] border-[#C9A961]/20 text-white max-w-lg">
                <DialogHeader>
                  <DialogTitle className="text-[#C9A961]">Nova Medição</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                    <Label className="text-gray-300">Tipo de Medição</Label>
                    <Select value={newMed.tipo} onValueChange={(v) => setNewMed({ ...newMed, tipo: v })}>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewMed({ ...newMed, dataPrevista: e.target.value })}
                      className="bg-[#0F1923] border-[#C9A961]/30 text-white mt-1"
                    />
                  </div>
                  <div>
                    <Label className="text-gray-300">Valor Esperado CEF (R$)</Label>
                    <Input
                      type="number"
                      step="0.01"
                      value={newMed.valorCef}
                      onChange={(e) => setNewMed({ ...newMed, valorCef: e.target.value })}
                  ...
                  <div className="flex gap-3 justify-end pt-2">
                    <Button variant="ghost" onClick={() => setOpenNew(false)} className="text-gray-400">
                      Cancelar
                    </Button>
                    <Button
                      ...
                      onClick={() => {
                        createMutation.mutate({
                          ...
                          dataPrevista: newMed.dataPrevista ? new Date(newMed.dataPrevista) : undefined,
                          valorCef: newMed.valorCef ? Math.round(parseFloat(newMed.valorCef) * 100) : undefined,
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Card className="bg-[#0F1923] border-[#C9A961]/20">
              <CardContent className="pt-4 pb-4">
                ...
                <p className="text-green-400 font-bold text-xl">{fmtBRL(totalRecebido)}</p>
              </CardContent>
            </Card>
            <Card className="bg-[#0F1923] border-[#C9A961]/20">
              <CardContent className="pt-4 pb-4">
                <p className="text-gray-400 text-xs mb-1">Lucro Estimado</p>
                <p className="text-yellow-400 font-bold text-xl">{fmtBRL(estimatedProfit)}</p>
              </CardContent>
            </Card>
          </div>
        )}
          ...
          <CardContent>
            {medicoes.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="text-sm mt-1">Clique em "Registrar Medição" para começar.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#C9A961]/20">
                      ...
                      <th className="text-left py-3 px-3 text-gray-400 font-medium">Data Prevista</th>
                      <th className="text-left py-3 px-3 text-gray-400 font-medium">Receb. CEF</th>
                      <th className="text-right py-3 px-3 text-gray-400 font-medium">Valor CEF</th>
                      <th className="text-left py-3 px-3 text-gray-400 font-medium">Data Transferência</th>
                      <th className="text-right py-3 px-3 text-gray-400 font-medium">Valor Transferido</th>
                      <th className="text-left py-3 px-3 text-gray-400 font-medium">Status</th>
                      <th className="text-right py-3 px-3 text-gray-400 font-medium">Pago Empreiteiro</th>
                      <th className="text-center py-3 px-3 text-gray-400 font-medium">Ações</th>
                    </tr>
                  </thead>
                  <tbody>
                    {medicoes.map((m) => {
                      const statusInfo = STATUS_MAP[m.status] || STATUS_MAP.pending;
                      const tipoLabel = MEDICAO_TIPOS.find(t => t.key === m.tipo)?.label || m.tipo;
                      return (
                        <tr key={m.id} className="border-b border-[#C9A961]/10 hover:bg-[#C9A961]/5 transition-colors">
                          <td className="py-3 px-3 font-medium text-white">{tipoLabel}</td>
                          <td className="py-3 px-3 text-gray-300">{fmtDate(m.dataPrevista)}</td>
                          <td className="py-3 px-3 text-gray-300">{fmtDate(m.dataRecebimentoCef)}</td>
                          <td className="py-3 px-3 text-right text-gray-300">{fmtBRL(m.valorCef)}</td>
                          <td className="py-3 px-3 text-gray-300">{fmtDate(m.dataTransferencia)}</td>
                          <td className="py-3 px-3 text-right text-green-400 font-medium">{fmtBRL(m.valorTransferido)}</td>
                          ...
                          <td className="py-3 px-3 text-right text-yellow-400">{fmtBRL(m.valorPagoEmpreiteiro)}</td>
                          <td className="py-3 px-3">
                            <div className="flex items-center gap-2 justify-center">
                              {m.status === "cef_paid" && (
                                <Button
                                  ...
                                  onClick={() => {
                                    const transferValue = prompt("Valor transferido pelo cliente (R$):");
                                    if (!transferValue) return;
                                    confirmReceiptMutation.mutate({
                                      id: m.id,
                                      valorTransferido: Math.round(parseFloat(transferValue) * 100),
                              ...
                              {m.status === "received" && (
                                <Dialog open={openPay === m.id} onOpenChange={(v) => { setOpenPay(v ? m.id : null); setPayAmount(""); }}>
                                  <DialogTrigger asChild>
                                    <Button
                                      size="sm"
                                      className="bg-yellow-600 hover:bg-yellow-500 text-white text-xs h-7"
                                    >
                                      <HardHat className="h-3 w-3 mr-1" /> Pagar Empreiteiro
                                    </Button>
                                  </DialogTrigger>
                                  <DialogContent className="bg-[#1A2332] border-[#C9A961]/20 text-white max-w-sm">
                                    <DialogHeader>
                                      <DialogTitle className="text-[#C9A961]">Pagar Empreiteiro</DialogTitle>
                                    </DialogHeader>
                                    <div className="space-y-4">
                                      <p className="text-gray-400 text-sm">Medição: <span className="text-white">{tipoLabel}</span></p>
                                      <div>
                                      ...
                                      <div className="flex gap-3 justify-end">
                                        <Button variant="ghost" onClick={() => setOpenPay(null)} className="text-gray-400">
                                          Cancelar
                                        </Button>
                                        <Button
                                          className="bg-yellow-600 hover:bg-yellow-500 text-white font-bold"
                                          disabled={payContractorMutation.isPending || !payAmount}
                                          onClick={() => {
                                            payContractorMutation.mutate({
                                              id: m.id,
                                              valorPagoEmpreiteiro: Math.round(parseFloat(payAmount) * 100),
                                            });
                                          }}
                                        >
                                          {payContractorMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Confirmar Pagamento"}
                                        </Button>
                                      </div>
                                    </div>
                                  </DialogContent>
        ...
        <Card className="bg-[#0F1923] border-[#C9A961]/20">
          <CardHeader>
            <CardTitle className="text-[#C9A961] flex items-center gap-2">
              <TrendingUp className="h-5 w-5" /> DRE — Demonstração de Resultados
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div className="bg-[#1A2332] rounded-lg p-4 border border-[#C9A961]/10">
                ...
                <p className="text-green-400 font-bold text-lg">{fmtBRL(totalRecebido)}</p>
              </div>
              <div className="bg-[#1A2332] rounded-lg p-4 border border-[#C9A961]/10">
                <p className="text-gray-400 text-xs mb-1">Pago Empreiteiros</p>
                <p className="text-red-400 font-bold text-lg">({fmtBRL(totalPagoEmpreiteiros)})</p>
              </div>
              <div className="bg-[#1A2332] rounded-lg p-4 border border-[#C9A961]/10">
              ...
              <div className="bg-[#1A2332] rounded-lg p-4 border border-[#C9A961]/10">
                ...
                <p className={`font-bold text-lg ${saldoProspecta >= 0 ? "text-[#C9A961]" : "text-red-400"}`}>
                  {fmtBRL(saldoProspecta)}
                </p>
              </div>
              <div className="bg-[#1A2332] rounded-lg p-4 border border-[#C9A961]/20 border-2">
                <p className="text-gray-400 text-xs mb-1">Lucro Estimado</p>
                <p className="text-[#C9A961] font-bold text-lg">{fmtBRL(estimatedProfit)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
