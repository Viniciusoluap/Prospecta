...
export default function MeusBilhetes() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <Badge className="bg-gray-600 text-white border-0">{status}</Badge>
        );
    }
  };
          ...
          {isLoading ? (
                    ...
                    <div className="grid md:grid-cols-3 gap-4 p-4 bg-[#2C3E50] rounded-lg border border-[#C9A961]/20">
                      <div>
                        ...
                        <p className="font-medium text-white text-lg">
                          {ticket.quantity} bilhete(s)
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-400">Valor Pago</p>
                        <p className="font-medium text-[#C9A961] text-lg">
                          {formatCurrency(ticket.totalPaid)}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-400">Método</p>
                        <p className="font-medium text-white text-lg uppercase">
                        ...
                        <Alert className="bg-yellow-500/10 border-yellow-500/30">
                          ...
                          <AlertDescription className="text-yellow-200">
                            ...
                            <p className="text-sm mb-3">
                              Complete o pagamento via PIX para confirmar sua
                    ...
                    {ticket.paymentStatus === "confirmed" && (
                      <div className="flex items-center gap-2 text-base text-[#00FF00] p-3 bg-[#00FF00]/10 rounded-lg border border-[#00FF00]/30">
                        ...
                        <span className="font-medium">
                          Bilhete confirmado! Boa sorte no sorteio!
                        </span>
                      </div>
                    )}
                  </CardContent>
            ...
            <Card className="bg-[#1A2332]/60 border-[#C9A961]/20 backdrop-blur-sm">
              <CardContent className="py-16 text-center">
                ...
                <Button
                  asChild
                  className="bg-[#00FF00] hover:bg-[#00dd00] text-black font-bold text-lg px-8 py-6 h-auto"
                >
                  <Link href="/sorteios">Ver Sorteios Ativos</Link>
                </Button>
              </CardContent>
            </Card>
          )}
