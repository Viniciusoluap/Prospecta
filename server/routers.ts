...
function generatePixBRCode(
  ...
  const payload = [
    ...
    {
      id: "26",
      value: `0014br.gov.bcb.pix01${pixKey.length.toString().padStart(2, "0")}${pixKey}`,
  ...
  for (const item of payload) {
    brcode +=
      item.id + item.value.length.toString().padStart(2, "0") + item.value;
  }
...
async function generatePixCode(
  amount: number,
  ticketNumber: string
): Promise<{ pixCopyPaste: string; pixQrCode: string }> {
  ...
  draws: router({
    ...
    performDraw: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const winnerIndex =
          parseInt(input.lotteryResult.slice(-2)) % confirmedTickets.length;
  ...
  tickets: router({
    ...
    confirmPayment: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const ticket = (await db.getTicketsByUserId(ctx.user.id)).find(
  ...
  budgetRequests: router({
    ...
    create: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const { sendEmail, budgetConfirmationTemplate } = await import(
          "./_core/email-smtp"
        );
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        return db.createBrokerCommission({
          ...
          installment1DueDate: input.dueDate1
            ? new Date(input.dueDate1)
            : undefined,
          installment2DueDate: input.dueDate2
            ? new Date(input.dueDate2)
            : undefined,
          installment3DueDate: input.dueDate3
            ? new Date(input.dueDate3)
            : undefined,
          installment4DueDate: input.dueDate4
            ? new Date(input.dueDate4)
            : undefined,
          status: "pending",
        } as any);
      }),
    ...
    create: protectedProcedure
      .input(
        z.object({
          ...
          status: z
            .enum(["available", "reserved", "sold", "construction"])
            .optional(),
          notes: z.string().optional(),
        })
      )
  ...
  partnerDistributions: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        return db.createPartnerDistribution({
          ...
          distributionAmount:
            input.amount?.toString() || input.distributionAmount || "0",
          notes: input.notes,
          status: "pending",
        } as any);
      }),
    ...
    markPaid: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        if (ctx.user.role !== "admin")
          throw new TRPCError({ code: "FORBIDDEN" });
        await db.updatePartnerDistribution(input.id, {
          status: "paid",
          paidAt: new Date(),
        } as any);
        return { success: true };
