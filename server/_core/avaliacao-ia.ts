 ...
 * Grupo Santa Fé (web/src/app/api/avaliacoes/sugestao/route.ts): Claude com
 * a ferramenta nativa de busca na web, ajustando o valor pelo checklist de
 * vistoria preenchido (documentação + condição física).
 */
...
function buildChecklistContext(raw: string): string {
  if (!raw) return "";
  let parsed: Partial<ChecklistData>;
  try {
    parsed = JSON.parse(raw) as Partial<ChecklistData>;
  } catch {
    return "";
  }
...
export async function gerarSugestaoValor(input: SugestaoInput): Promise<SugestaoResult> {
  ...
  const tipoDesc = TIPO_LABELS[input.tipo] ?? input.tipo;
  const areaDesc = input.areaConstruida ? `${input.areaConstruida} m² construídos` : "";
  const terrenoDesc = input.areaTerreno ? `, ${input.areaTerreno} m² de terreno` : "";
  const quartosDesc = input.quartos ? `, ${input.quartos} quartos` : "";
  const banheirosDesc = input.banheiros ? `, ${input.banheiros} banheiros` : "";

  const checklistCtx = buildChecklistContext(input.caracteristicas ?? "");

  const prompt = `Você é um perito avaliador imobiliário especializado no mercado do Pará e Maranhão (regiões de Canaã dos Carajás e Imperatriz).

Imóvel a avaliar:
- Endereço: ${input.endereco}, ${input.bairro}, ${input.cidade}/${input.estado}
- Tipo: ${tipoDesc}
- Características: ${areaDesc}${terrenoDesc}${quartosDesc}${banheirosDesc}
${checklistCtx}

Faça UMA busca web por imóveis similares em ${input.cidade} - ${input.bairro} (ZAP, VivaReal, OLX ou Imovelweb). Com os dados encontrados, calcule o valor de mercado pelo método comparativo direto e ajuste conforme a condição real do imóvel descrita no checklist acima.
...
Preencha com no máximo 4 comparáveis. Todos os valores numéricos devem ser inteiros sem decimais. No campo "metodologia" explique brevemente como o checklist influenciou o valor final.`;
  ...
  return JSON.parse(jsonText.slice(start, end + 1)) as SugestaoResult;
}
