// Cronograma Físico-Financeiro — curva S física (medições reais em obra) x
// financeira (desembolso linear projetado a partir da duração de obra).
// Função pura, portada do Grupo Santa Fé sem alteração de lógica.
//
// Adaptação deliberada (mesma classe das S-05/S-06/S-10): no Santa Fé, a
// duração de obra vem automaticamente da aba de Viabilidade Econômica, que
// ainda não existe no Prospecta — por isso `resumoCronogramaObra` recebe a
// duração como parâmetro explícito (já era opcional no código original), e
// a UI desta story pede esse valor manualmente.
